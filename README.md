...
- Streaming: sealed prefix plus mutable tail buffer. Sealed prefixes emit
  `INSERT` and `SEAL` patches with offset, length, and content hash metadata.
...
- Stable-prefix detection is conservative and block-boundary based; finer
  per-block patching is pending.
- HTML policy is represented in options but not yet enforced beyond cmark-gfm
  IR emission.
