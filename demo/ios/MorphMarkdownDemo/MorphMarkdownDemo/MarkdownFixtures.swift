enum MarkdownFixtures {
	static func create(validURL: String, invalidURL: String) -> String {
		return [
			"# Streaming Markdown on iOS\n\n",
			...
			"view.mathRenderer\t= MathJaxMathRenderer(fontPath: path)\n",
			"```\n\n",
			"---\n\n",
			table(validURL: validURL, invalidURL: invalidURL),
			"Valid image:\n\n![generated demo](\(validURL) \"generated\")\n\n",
			"Invalid image:\n\n![missing demo](\(invalidURL) \"missing\")\n\n",
			chineseSection(validURL: validURL, invalidURL: invalidURL)
		].joined()
	}

	static func tableDemo(validURL: String, invalidURL: String) -> String {
		return [
			"# Table Rendering\n\n",
			table(validURL: validURL, invalidURL: invalidURL),
			"## 中文表格\n\n",
			chineseTable(validURL: validURL, invalidURL: invalidURL)
		].joined()
	}

	private static func table(validURL: String, invalidURL: String) -> String {
		return [
			"## Dynamic table\n\n",
			"| feature | status |\n",
			"|:---|:---:|\n",
			...
			"| valid image ![generated](\(validURL)) | decoded bitmap in cell |\n",
			"| invalid image ![missing](\(invalidURL)) | error placeholder in cell |\n",
	...
	private static func chineseSection(validURL: String, invalidURL: String) -> String {
		return [
			"# 中文排版 HetiLike\n\n",
			...
			"## 二级标题：中文标题间距\n\n",
			"中文段落通常比英文更依赖稳定的行高和段落节奏。HetiLike 主题会使用阅读型标题层级，",
			"并让表格、列表、引用都跟正文网格靠齐。\n\n",
			"### 三级标题：列表与任务\n\n",
			"- 第一层无序列表使用实心圆，适合普通条目。\n",
			"  - 第二层列表使用空心圆，用于补充说明。\n",
	...
	private static func chineseTable(validURL: String, invalidURL: String) -> String {
		return [
			"| 场景 | 效果 |\n",
			"|:---|:---|\n",
			...
			"| 表格内有效图片 | ![generated](\(validURL)) |\n",
			"| 表格内无效图片 | ![missing](\(invalidURL)) |\n",
